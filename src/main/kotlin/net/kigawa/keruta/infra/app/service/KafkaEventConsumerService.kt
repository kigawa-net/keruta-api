...
open class KafkaEventConsumerService(
    ...
    fun consumeSessionEvents(
        ...
        try {
            logger.debug(
                "Received session event from topic '{}' [partition: {}, offset: {}]: {}",
                topic,
                partition,
                offset,
                message,
    ...
    fun consumeWorkspaceEvents(
        ...
        try {
            logger.debug(
                "Received workspace event from topic '{}' [partition: {}, offset: {}]: {}",
                topic,
                partition,
                offset,
                message,
    ...
    fun consumeTaskEvents(
        ...
        try {
            logger.debug(
                "Received task event from topic '{}' [partition: {}, offset: {}]: {}",
                topic,
                partition,
                offset,
                message,
    ...
    override fun handleSessionEvent(event: Event) {
        when (event) {
            is SessionStatusChangedEvent -> {
                ...
                logger.info(
                    "Received session event: type={}, eventId={}",
    ...
    override fun handleWorkspaceEvent(event: Event) {
        when (event) {
            is WorkspaceCreatedEvent -> {
                logger.info(
                    "Workspace created: workspaceId={}, sessionId={}",
                    event.workspaceId,
                    event.sessionId,
                )
            }
            is WorkspaceStartedEvent -> {
                logger.info(
                    "Workspace started: workspaceId={}, sessionId={}",
                    event.workspaceId,
                    event.sessionId,
                )
            }
            is WorkspaceStoppedEvent -> {
                logger.info(
                    "Workspace stopped: workspaceId={}, sessionId={}, reason={}",
                    event.workspaceId,
                    event.sessionId,
                    event.reason,
                )
                ...
                logger.info(
                    "Received workspace event: type={}, eventId={}",
                    event::class.simpleName,
                    event.eventId,
                )
            }
    ...
    override fun handleTaskEvent(event: Event) {
        when (event) {
            ...
            is TaskStatusChangedEvent -> {
                logger.info(
                    "Task status changed: taskId={}, status={} -> {}",
                    event.taskId,
                    event.previousStatus,
                    event.newStatus,
                )
                ...
                logger.info(
                    "Received task event: type={}, eventId={}",
                    event::class.simpleName,
                    event.eventId,
                )
            }
