...
class SessionLogController(
    ...
    @Operation(
        ...
        description = "Retrieves logs for a specific session with optional filters",
    )
    suspend fun getSessionLogs(
        ...
        logger.info(
            "Getting session logs: sessionId={}, filters=level:{}, source:{}, action:{}",
            sessionId,
            level,
            source,
            action,
    ...
    @Operation(
        summary = "Get session log count",
        description = "Gets the total count of logs for a session with optional filters",
    )
